...
# Defensive: a caller that passes a trailing backslash inside quotes (cmd's
# %~dp0 + \" ) can leave a literal quote at the end of the path. Trim it.
...
function Start-BoujoyServices {
    ...
    try {
        ...
        $children.Add((Start-BoujoyChild -Name "knowledge" -FilePath $Layout.DshCommand `
            -Arguments @("web", "--host", "127.0.0.1", "--port", "3280", "--no-open") -WorkingDirectory $Layout.Vault `
        ...
        $children.Add((Start-BoujoyChild -Name "clean" -FilePath $Layout.DshCommand `
            -Arguments @("web", "--host", "127.0.0.1", "--port", "3281", "--no-open") -WorkingDirectory $env:USERPROFILE `
        ...
        Wait-BoujoyEndpoint -Label "Clean Harness" -Url "http://127.0.0.1:3281/"
        Wait-BoujoyEndpoint -Label "Boujoy gateway" -Url "http://127.0.0.1:8876/api/health"
...
try {
    ...
    if (Test-BoujoyEndpoint "http://127.0.0.1:8876/api/health") {
